...
export default function CustomerMessaging() {
  ...
  const fetchDisplayName = async (userId) => {
    try {
      ...
      console.error(`[CustomerMessaging] Error fetching display name for ${userId}:`, error)
    }
    return `User ${userId}`
  }

  // Create default conversations for customer using order participants
  const createDefaultConversations = async () => {
    if (!orderContext?.orderContext || !currentUser || !orderContext.orderId) {
      return []
    }

    console.log(`[CustomerMessaging] Creating default conversations for order ${orderContext.orderId}`)
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[CustomerMessaging] Order participants:`, participants)
        ...
        console.log('[CustomerMessaging] Failed to fetch order participants, falling back to order context')
        
        // Fallback: Add restaurant owner using order context
      ...
      console.error('[CustomerMessaging] Error fetching order participants:', error)
    }
  ...
  const loadConversations = async () => {
    ...
    setLoading(true)
    try {
      console.log(`[CustomerMessaging] Loading conversations for customer ${currentUser.id}`)
      ...
      if (response.ok) {
        ...
        console.log(`[CustomerMessaging] Found ${data.length} messages`)
        ...
        console.log(`[CustomerMessaging] Processed ${realConversations.length} real conversations`)
      } else {
        console.log('[CustomerMessaging] No conversations found or error loading')
      }
      ...
      console.log(`[CustomerMessaging] Total conversations: ${allConversations.length}`)
      
    } catch (error) {
      console.error('[CustomerMessaging] Error loading conversations:', error)
  ...
  const loadMessages = async (partnerId) => {
    ...
    try {
      console.log(`[CustomerMessaging] Loading messages between ${currentUser.id} and ${partnerId}`)
      const response = await fetch(`http://localhost:8084/api/users/${currentUser.id}/conversations/${partnerId}`)
      
      if (response.ok) {
        const data = await response.json()
        console.log(`[CustomerMessaging] Loaded ${data.length} messages`)
        setMessages(data)
      } else {
        console.log('[CustomerMessaging] No messages found for this conversation')
        setMessages([])
      }
    } catch (error) {
      console.error('[CustomerMessaging] Error loading messages:', error)
      setMessages([])
    }
  }
  ...
  const sendMessage = async () => {
    ...
    try {
      console.log(`[CustomerMessaging] Sending message from ${currentUser.id} to ${selectedChat.partnerId}`)
      ...
      if (response.ok) {
        console.log('[CustomerMessaging] Message sent successfully - triggering Kafka event')
        setMessageInput('')
        ...
        console.error('[CustomerMessaging] Failed to send message')
      }
    } catch (error) {
      console.error('[CustomerMessaging] Error sending message:', error)
    }
  }

  // Handle conversation selection
  const selectConversation = (conversation) => {
    console.log(`[CustomerMessaging] Selected conversation with ${conversation.name}`)
    ...
    <SidebarProvider defaultOpen={false}>
      ...
      <main className="flex-1 bg-gray-50 min-h-screen">
        ...
        <div className="container mx-auto max-w-7xl px-4 py-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-6">Customer Messaging</h1>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {conversation.role && (
                            <p className="text-xs text-blue-600">{conversation.role}</p>
                          )}
              ...
              {selectedChat ? (
                <>
                  ...
                  <div className="p-4 border-b">
                    ...
                    {selectedChat.role && (
                      <p className="text-sm text-blue-600">{selectedChat.role}</p>
                    )}
                  </div>
