...
export default function BusinessOwnerMessaging() {
  ...
  const fetchDisplayName = async (userId) => {
    try {
      ...
      console.error(`[BusinessOwnerMessaging] Error fetching display name for ${userId}:`, error)
    }
    return `User ${userId}`
  }

  // Create default conversations for business owner using order participants
  const createDefaultConversations = async () => {
    if (!orderContext?.orderContext || !currentUser || !orderContext.orderId) {
      return []
    }

    console.log(`[BusinessOwnerMessaging] Creating default conversations for order ${orderContext.orderId}`)
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[BusinessOwnerMessaging] Order participants:`, participants)
        ...
        // Add rider conversation if available
        if (participants.rider && participants.rider.id !== currentUser.id) {
          defaultConversations.push({
            id: participants.rider.id.toString(),
            partnerId: participants.rider.id,
            name: participants.rider.displayName,
            role: participants.rider.role,
            lastMessage: {
              content: "Start your conversation by send message now!",
              timestamp: new Date()
            },
        ...
        console.log('[BusinessOwnerMessaging] Failed to fetch order participants, falling back to order context')
        
        // Fallback: Add customer using order context
      ...
      console.error('[BusinessOwnerMessaging] Error fetching order participants:', error)
    }
  ...
  const loadConversations = async () => {
    ...
    setLoading(true)
    try {
      console.log(`[BusinessOwnerMessaging] Loading conversations for business owner ${currentUser.id}`)
      ...
      if (response.ok) {
        ...
        console.log(`[BusinessOwnerMessaging] Found ${data.length} messages`)
        ...
        console.log(`[BusinessOwnerMessaging] Processed ${realConversations.length} real conversations`)
      } else {
        console.log('[BusinessOwnerMessaging] No conversations found or error loading')
      }
      ...
      console.log(`[BusinessOwnerMessaging] Total conversations: ${allConversations.length}`)
      
    } catch (error) {
      console.error('[BusinessOwnerMessaging] Error loading conversations:', error)
  ...
  const loadMessages = async (partnerId) => {
    ...
    try {
      console.log(`[BusinessOwnerMessaging] Loading messages between ${currentUser.id} and ${partnerId}`)
      const response = await fetch(`http://localhost:8084/api/users/${currentUser.id}/conversations/${partnerId}`)
      
      if (response.ok) {
        const data = await response.json()
        console.log(`[BusinessOwnerMessaging] Loaded ${data.length} messages`)
        setMessages(data)
      } else {
        console.log('[BusinessOwnerMessaging] No messages found for this conversation')
        setMessages([])
      }
    } catch (error) {
      console.error('[BusinessOwnerMessaging] Error loading messages:', error)
      setMessages([])
    }
  }
  ...
  const sendMessage = async () => {
    ...
    try {
      console.log(`[BusinessOwnerMessaging] Sending message from ${currentUser.id} to ${selectedChat.partnerId}`)
      ...
      if (response.ok) {
        console.log('[BusinessOwnerMessaging] Message sent successfully - triggering Kafka event')
        setMessageInput('')
        ...
        console.error('[BusinessOwnerMessaging] Failed to send message')
      }
    } catch (error) {
      console.error('[BusinessOwnerMessaging] Error sending message:', error)
    }
  }

  // Handle conversation selection
  const selectConversation = (conversation) => {
    console.log(`[BusinessOwnerMessaging] Selected conversation with ${conversation.name}`)
    ...
    <SidebarProvider defaultOpen={false}>
      ...
      <main className="flex-1 bg-gray-50 min-h-screen">
        ...
        <div className="container mx-auto max-w-7xl px-4 py-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-6">Business Owner Messaging</h1>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {conversation.role && (
                            <p className="text-xs text-green-600">{conversation.role}</p>
                          )}
              ...
              {selectedChat ? (
                <>
                  ...
                  <div className="p-4 border-b">
                    ...
                    {selectedChat.role && (
                      <p className="text-sm text-green-600">{selectedChat.role}</p>
                    )}
                  </div>
